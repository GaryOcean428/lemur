...
export default function DeepResearchPanel() {
  ...
  const [researchResults, setResearchResults] =
    useState<DeepResearchResponse | null>(null);
  const [extractedContent, setExtractedContent] =
    useState<UrlContentResponse | null>(null);
  ...
  const handleResearch = async () => {
    ...
    if (!isProUser) {
      toast({
        ...
        description:
          "Advanced research is a Pro tier feature. Please upgrade your subscription to access this feature.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleExtractContent = async () => {
    ...
    if (!isProUser) {
      toast({
        title: "Pro subscription required",
        description:
          "Content extraction is a Pro tier feature. Please upgrade your subscription to access this feature.",
        variant: "destructive",
      });
      return;
    }
  ...
  const getPreviousQueries = () => {
    ...
    const previousQueries = JSON.parse(
      localStorage.getItem("previousQueries") || "[]",
    );
    ...
    <div className="max-w-3xl mx-auto p-4">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Pro-tier deep research capabilities for comprehensive information
          gathering
        </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-600 dark:text-yellow-400">
                    Advanced research features require a Pro subscription.
                    Upgrade to access these powerful tools.
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          {researchResults && (
            <Card className="overflow-hidden">
              <CardHeader className="pb-0">
                <CardTitle>Research Results: {researchResults.query}</CardTitle>
              ...
              <CardContent>
                <div className="mt-4">
                  ...
                  {researchResults.topic_clusters &&
                    Object.keys(researchResults.topic_clusters).length > 0 && (
                      <div className="mb-6">
                        <h3 className="text-md font-semibold mb-2">
                          Topic Clusters
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {Object.entries(researchResults.topic_clusters).map(
                            ([topic, keywords]) => (
                              <div
                                key={topic}
                    ...
                    {extractedContent.metadata &&
                      Object.keys(extractedContent.metadata).length > 0 && (
                        <div className="mb-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {Object.entries(extractedContent.metadata).map(
                              ([key, value]) => {
