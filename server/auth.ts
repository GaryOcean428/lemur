...
export function setupAuth(app: Express) {
  ...
  // Debug endpoint to test session and auth
  app.get("/api/auth-debug", (req, res) => {
    // Return detailed information about the request's authentication state
    res.json({
      ...
      sessionData: req.session ? {
        ...
        // Don't include passport data directly for security
        hasPassport: !!req.session.passport,
        passportKeys: req.session.passport ? Object.keys(req.session.passport) : [],
        otherKeys: Object.keys(req.session).filter(key => key !== 'cookie' && key !== 'passport')
      } : null,
      user: req.isAuthenticated() ? {
        id: req.user.id,
        username: req.user.username,
        subscriptionTier: req.user.subscriptionTier,
        // Don't send the password or other sensitive data
      } : null
    });
  });
    ...
    try {
      ...
      if (!user) {
        return res.status(401).json({
          ...
          debug: { userLookup: "failed", username }
        });
      }
      ...
      if (!passwordValid) {
        return res.status(401).json({
          ...
          debug: { 
            ...
            // First few chars of stored hash for verification
